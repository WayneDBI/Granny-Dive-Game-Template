...
function nextLevelGo()
	...
	-- Tell the engine the user has now played MULTIPLE levels
	multiLevelsPlayed = multiLevelsPlayed + 1
...
function scene:createScene( event )
	...
	buttonMenu.x = (_w/2)-80; buttonMenu.y = 320+iPhone5Offset
	...
	buttonReplay.x = (_w/2); buttonReplay.y = 320+iPhone5Offset
	screenGroup:insert( buttonReplay )

	local buttonNextLevel = widget.newButton{
		sheet = imageSheet, defaultFrame = 67, overFrame = 67,
		overColor = {128,128,128,100}, width = 75, height = 69,
		onRelease = nextLevelGo, }
	--buttonNextLevel:setReferencePoint(display.CenterReferencePoint)
	buttonNextLevel.x = (_w/2)+80; buttonNextLevel.y = 320+iPhone5Offset
	screenGroup:insert( buttonNextLevel )

	local panelHeader = display.newSprite( imageSheet, animationSequenceData )
	panelHeader:setSequence( "infoCompleted" )
...
function scene:destroyScene( event )
	print( "((destroying overlayWin view))" )
end
